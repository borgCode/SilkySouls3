...
namespace SilkySouls3.Services
{
    public class UtilityService
    {
        ...
        {
            var bonfireFlagBasePtr = (IntPtr) _memoryIo.ReadInt64((IntPtr)_memoryIo.ReadInt64(Offsets.EventFlagMan.Base));
            _memoryIo.SetBitValue(bonfireFlagBasePtr + Offsets.EventFlagMan.CoiledSword, Offsets.EventFlagMan.CoiledSwordBitFlag, true);
            
            var lastBonfireAddr = (IntPtr)_memoryIo.ReadInt64(Offsets.GameMan.Base) + Offsets.GameMan.LastBonfire;
            _memoryIo.WriteInt32(lastBonfireAddr, warpLocation.BonfireID + 1000);
            
            byte[] warpBytes = AsmLoader.GetAsmBytes("WarpCall");
            byte[] bytes = BitConverter.GetBytes(Offsets.LuaEventMan.Base.ToInt64());
            Array.Copy(bytes, 0, warpBytes, 2, 8);
            bytes = BitConverter.GetBytes(Offsets.Funcs.Warp);
            ...
            if (warpLocation.HasCoordinates)
            {
                ...
                var coordsOrigin = Offsets.Hooks.WarpCoordWrite;
                ...
                var angleOrigin = Offsets.Hooks.WarpCoordWrite -0x20;
